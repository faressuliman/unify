...
import { Send, Paperclip, MessageCircle, Loader2, ArrowLeft, ArrowRight, X } from 'lucide-react';
import { chatApi, type BackendChat, type BackendChatUser, type BackendMessage } from '../lib/api';
import { useAuth } from '../context/AuthContext';
import { useLanguage } from '../context/LanguageContext';
import PageHeader from '../components/ui/PageHeader';
import { getSocket } from '@/lib/socket';

function getOtherUser(chat: BackendChat, currentUserId: string): BackendChatUser | null {
  ...
  const initiatorId = initiatorObj?._id || (typeof initiator === 'string' ? initiator : '');
...
export default function Chat() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      setSending(true);
      const res = await chatApi.sendMessage(activeChatId, newMsg.trim() || undefined, attachment, token);
      if (res.message) {
        setMessages((prev) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col pt-8 pb-12" dir={isRTL ? 'rtl' : 'ltr'}>
      <PageHeader
        ...
        subtitle={isRTL ? 'تحدث مع أصحاب المنشورات بعد الموافقة على المطالبة.' : 'Chat with post owners after a claim has been approved.'}
        showArrow
      />
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs grid grid-cols-1 md:grid-cols-[280px_1fr] h-[70vh] min-h-[500px]">
          ...
          <aside className={`border-${isRTL ? 'l' : 'r'} border-slate-100 flex flex-col ${activeChatId ? 'hidden md:flex' : 'flex'}`}>
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="font-bold text-tertiary">{isRTL ? 'المحادثات' : 'Chats'}</h2>
            </div>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-500 truncate">
                              {new Date(chat.createdAt).toLocaleDateString(isRTL ? 'ar-EG' : 'en-US', {
                                month: 'short',
                                day: 'numeric',
                              })}
                            </p>
          ...
          {/* Conversation */}
          <section className={`flex flex-col ${activeChatId ? 'flex' : 'hidden md:flex'}`}>
            {!activeChat ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center text-slate-500 px-6">
                ...
                <p className="text-sm">
                  {isRTL ? 'اختر محادثة لعرض الرسائل.' : 'Select a chat to view messages.'}
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-3 bg-slate-50/40">
                  {messagesLoading ? (
                    ...
                    <p className="text-center text-sm text-slate-500 py-10">
                      {isRTL ? 'لا توجد رسائل بعد. ابدأ المحادثة!' : 'No messages yet. Say hi!'}
                    </p>
                  ) : (
                    messages.map((m) => {
