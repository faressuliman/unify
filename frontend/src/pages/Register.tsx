...
const getPasswordStrength = (
  password: string,
  content: PasswordStrengthContent,
) => {
  let score = 0;
  ...
  if (password.length > 0) {
    if (score === 1) {
      ...
      text = content.pwdWeak;
    } else if (score === 2) {
      color = "bg-orange-400";
      textColor = "text-orange-400";
      text = content.pwdFair;
    } else if (score === 3) {
      color = "bg-blue-500";
      textColor = "text-blue-500";
      text = content.pwdGood;
    } else if (score === 4) {
      color = "bg-green-500";
      textColor = "text-green-500";
      text = content.pwdStrong;
    }
  }
...
const Register = () => {
  ...
  const localizeError = (message: string) => {
    return (
      content.validation[message as keyof typeof content.validation] ?? message
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. التأكد من رفع الصور أولاً
    if (!formData.idPicture || !formData.selfiePicture) {
      toast.error(
        isRTL ? "برجاء رفع الصور أولاً" : "Please upload images first",
      );
      return;
    }

    // 2. الـ Validation الأساسي بتاع Zod
    ...
    try {
      // بنجهز الـ FormData اللي هتروح لسيرفر البايثون (AI)
      const aiFormData = new FormData();
      aiFormData.append("idPicture", formData.idPicture);
      aiFormData.append("selfiePicture", formData.selfiePicture);
      aiFormData.append("enteredName", formData.fullName.trim()); // بنبعت الاسم عشان يترجم ويتقارن
      aiFormData.append("enteredYear", formData.birthDate.split("-")[0]); // بنبعت السنة

      // --- الخطوة الأولى: التحقق من تطابق الوجه ---
      toast.loading(isRTL ? "جاري مطابقة الوجه..." : "Verifying face...");
      const faceResponse = await fetch("http://127.0.0.1:8000/verify-face", {
        method: "POST",
        body: aiFormData,
      });
      ...
      if (!faceResult.verified) {
        toast.error(
          isRTL ? "الوجه لا يطابق صورة البطاقة!" : "Face does not match!",
        );
        setIsSubmitting(false);
        return;
      }

      // --- الخطوة الثانية: قراءة بيانات البطاقة والمطابقة (OCR + Translation) ---
      toast.loading(
        isRTL ? "جاري التحقق من بيانات البطاقة..." : "Verifying ID data...",
      );
      const ocrResponse = await fetch("http://127.0.0.1:8000/extract-id-data", {
        method: "POST",
        body: aiFormData, // بنبعت نفس الـ FormData لأن فيها كل البيانات
      });
      ...
      // هنا بنشيك على النتيجة اللي جاية من الـ Backend اللي فيه الترجمة
      if (!ocrResult.is_name_match) {
        toast.error(
          isRTL ? "الاسم غير متطابق مع البطاقة!" : "Name does not match ID!",
        );
        setIsSubmitting(false);
        return;
      }

      if (!ocrResult.is_year_match) {
        toast.error(
          isRTL
            ? "تاريخ الميلاد لا يتطابق مع البطاقة!"
            : "Birth date mismatch!",
        );
        setIsSubmitting(false);
        return;
      ...
      // لو وصلنا هنا يبقى كل حاجة تمام (الوجه متطابق والبيانات صح)
      toast.success(
        isRTL
      ...
      // --- الخطوة الثالثة: إنشاء الحساب في الـ Node.js ---
      await register({
        ...
        selfiePicture: formData.selfiePicture,
  ...
  return (
    <div
      className={`flex min-h-[calc(100vh-80px)] bg-[#f8fafc] font-sans text-gray-900 relative overflow-hidden ${isRTL ? "dir-rtl" : ""}`}
      dir={isRTL ? "rtl" : "ltr"}
    >
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <FormInput
                  ...
                  className={
                    errors.fullName
                      ...
                      : "border-gray-200/80 focus:border-secondary"
                  }
                />
                <ErrorMessage
                  msg={errors.fullName}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <LocalizedDateInput
                    ...
                    isRTL={isRTL}
                    placeholder={
                      content.birthDatePlaceholder || content.birthDate
                    }
                  />
                  <ErrorMessage
                      ...
                      return (
                        <>
                          <div className="flex justify-between items-center mb-1.5">
                            <span
                              className={`text-xs font-bold ${strength.textColor}`}
                            >
                              {strength.text}
                            </span>
              ...
              <div className="text-start mt-6">
                ...
                <ImageUpload
                  ...
                  title="Upload Selfie"
                  dragDropText="Take or upload a clear selfie"
                  subtitle="Face must be visible clearly"
                  buttonText="Browse Files"
                  changeText="Change"
                  removeText="Remove"
                />
                <ErrorMessage
                  msg={errors.selfiePicture}
                  className="text-[11px] sm:text-xs"
                />
              </div>
            ...
            <p className="text-center text-sm text-gray-500 mt-8 font-medium">
              ...
              <Link
                to="/login"
                className={`text-tertiary font-bold hover:underline hover:decoration-secondary transition-all ${isRTL ? "mr-1" : "ml-1"}`}
              >
                {content.signIn}{" "}
                {isRTL ? (
                  <ArrowUpLeft className="w-4 h-4 text-secondary inline-block align-middle ml-1" />
                ) : (
                  <ArrowUpRight className="w-4 h-4 text-secondary inline-block align-middle ml-1" />
                )}
              </Link>
            </p>
          </motion.div>
