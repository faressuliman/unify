    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // Location is optional, so we just log the error
        },
      );
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ageUnit) {
      ...
      if (formData.ageUnit === "years" && (ageNum < 1 || ageNum > 90)) {
        ...
        newErrors.age =
          language === "ar"
            ? "الأشهر يجب أن تكون بين 1 و 11"
            : "Months must be between 1 and 11";
      } else if (formData.ageUnit === "days" && (ageNum < 1 || ageNum > 30)) {
        newErrors.age =
          language === "ar"
            ? "الأيام يجب أن تكون بين 1 و 30"
    ...
    if (formData.postType === "missing") {
      ...
      if (
        ...
        !/^\d{11}$/.test(formData.reporterPhone.trim())
      ) {
    ...
    if (!token) {
      setSubmitError(
        isRTL
          ? "يجب تسجيل الدخول أولاً لإنشاء بلاغ."
          : "You need to log in first to create a post.",
      );
      return;
    }
    ...
    try {
      const response = await postApi.createPost(
        {
          ...
          lastSeenLocation:
            formData.postType === "missing"
              ...
              : undefined,
          lastSeenDate:
            formData.postType === "missing" ? formData.lastSeenDate : undefined,
          foundLocation:
            formData.postType === "found"
              ? formData.foundLocation.trim()
              : undefined,
          affiliation:
            formData.postType === "found" && formData.affiliation
              ? formData.affiliation
              : undefined,
          organizationName:
            formData.postType === "found" &&
            ...
            formData.affiliation !== "none"
              ...
              : undefined,
          reporterPhone:
            formData.postType === "found" && formData.reporterPhone.trim()
              ? formData.reporterPhone.trim()
              : undefined,
      ...
      toast.success(
        ...
        {
          description: isRTL
            ? "يمكنك عرضه الآن في صفحة البحث."
            : "You can view it now on the search page.",
          action: {
      ...
      >
        ...
        <div className="max-w-400 mx-auto px-6 lg:px-12 w-full">
          ...
          <form
            ...
            className="grid gap-8 lg:grid-cols-2 lg:items-start mt-6"
          >
            ...
            <motion.div className="space-y-6 min-w-0" variants={itemVariants}>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 font-sans">
                    <div>
                      <FormInput
                        ...
                        className={
                          errors.firstName
                            ? "border-red-400 focus:ring-1 focus:ring-red-500/20"
                            : "font-sans"
                        }
                      />
                      <ErrorMessage
                          ...
                          <FormInput
                            ...
                            onChange={(e) => {
                              ...
                              if (val !== "") {
                                ...
                                if (formData.ageUnit === "years" && num > 90)
                                  return;
                                if (formData.ageUnit === "months" && num > 11)
                                  return;
                                if (formData.ageUnit === "days" && num > 30)
                                  return;
                              }
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    />
                    <ErrorMessage
                      msg={errors.clothingDescription}
                      className="text-[12px] mt-1 text-red-500"
                    />
                  </div>
                </div>
                    ...
                    <>
                      ...
                      <div>
                        <LocalizedDateInput
                          ...
                          onChange={(val) =>
                            handleSelectChange("lastSeenDate", val)
                          }
                          placeholder={t.lastSeenDatePlaceholder}
                          isRTL={isRTL}
                        />
                        <ErrorMessage
                          msg={errors.lastSeenDate}
                          className="text-[12px] mt-1 text-red-500"
                        />
                      </div>
                    </>
                    ...
                    <>
                      ...
                      <div className="rounded-xl border border-slate-200 p-4 min-w-0">
                        <SelectMenu
                          id="affiliation"
                          label={
                            <span className="block whitespace-normal wrap-break-word leading-snug">
                              {t.affiliatedQuestion}{" "}
                          ...
                          onChange={(val) =>
                            handleSelectChange("affiliation", val)
                          }
                          isRTL={isRTL}
                        />
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
                ...
                <ErrorMessage
                  msg={errors.photo}
                  className="text-[12px] mt-2 text-red-500"
                />
              </div>
