...
const Poster = () => {
  ...
  const [contactType, setContactType] = useState<"mobile" | "email" | "both">(
    "mobile",
  );
  ...
  const previewImage =
    photoDataUrl || (photo ? URL.createObjectURL(photo) : "");
  ...
  const handleDownloadAction = async () => {
    ...
    if (!ageUnit)
      errors.push(language === "ar" ? "الوحدة مطلوبة" : "Age unit is required");
    ...
    if (contactType === "mobile" && !contactPhone.trim())
      errors.push(t("poster.contactReq"));
    if (contactType === "email" && !contactEmail.trim())
      errors.push(t("poster.contactReq"));
    if (
      contactType === "both" &&
      ...
      >
        <motion.div variants={itemVariants}>
          <PageHeader
            ...
            showArrow={true}
            titleClassName={
              !isRTL ? "tracking-[0.08em] sm:tracking-widest" : ""
            }
          />
        </motion.div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 font-sans">
                    ...
                    <FormInput
                      ...
                      suffix={
                        <span
                          className={`pointer-events-none absolute top-1/2 -translate-y-1/2 text-sm font-bold text-slate-400 ${isRTL ? "left-4" : "right-4"}`}
                        >
                          {t("poster.heightUnit")}
                        </span>
                      }
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label
                      id="contactTips"
                      className="mb-4 block text-start text-sm font-bold text-slate-600 font-sans"
                    >
                      {t("poster.contactTips")}
                    </label>
                    <SegmentedControl
                      className="mb-4"
                      value={contactType}
                      onChange={(val) => {
                        setContactType(val as "mobile" | "email" | "both");
                      }}
                      options={[
                        { value: "mobile", label: t("poster.mobile") },
                        { value: "email", label: t("poster.email") },
                        { value: "both", label: t("poster.both") },
                      ]}
                    />
                    <div className="space-y-2 text-start">
                      {(contactType === "mobile" || contactType === "both") && (
                        <FormInput
                          id="contactPhone"
                          label={null}
                          dir="ltr"
                      ...
                      {(contactType === "email" || contactType === "both") && (
                        <FormInput
                          ...
                          placeholder={t("poster.emailPlaceholder")}
                          className="h-14 px-4 py-3 text-base sm:text-lg text-center font-bold tracking-wide font-sans"
                        />
                      )}
                    </div>
              ...
              >
                ...
                <div className="relative w-full aspect-video bg-slate-200 flex items-center justify-center">
                  <AnimatePresence mode="wait">
                    {previewImage ? (
                      ...
                      >
                        ...
                        />
                        <span className="text-sm font-bold opacity-30 uppercase tracking-[0.2em] font-sans">
                          {t("poster.noPhoto")}
                        </span>
                      </motion.div>
                    )}
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-slate-900 via-slate-900/40 to-transparent flex flex-col justify-end p-8 pt-20">
                    <h3
                      className="text-4xl font-black text-white drop-shadow-xl font-sans leading-tight"
                ...
                <div className="p-16 bg-white flex flex-col gap-16">
                  <div className="grid grid-cols-2 gap-16 font-sans">
                    <div className="flex flex-col">
                      ...
                      >
                        {age && ageUnit
                          ? `${age} ${ageUnit === "years" ? (language === "ar" ? "سنوات" : "Years") : ageUnit === "months" ? (language === "ar" ? "شهر" : "Months") : language === "ar" ? "يوم" : "Days"}`
                          : t("poster.notProvided")}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span
