...
export default function EditProfileModal({ isOpen, onOpenChange, profile, onSuccess, isRTL }: EditProfileModalProps) {
  const { token } = useAuth();
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm pt-2">
                 <ImageUpload
                   ...
                   title={isRTL ? "صورة الهوية (اختياري)" : "ID Picture (Optional)"}
                   dragDropText=""
                   subtitle={isRTL ? "قم بتحديث صورة الهوية الخاصة بك إذا لزم الأمر" : "Update your ID picture if needed"}
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors font-medium outline-none"
                >
                   {isRTL ? 'إلغاء' : 'Cancel'}
                </button>
              </div>
