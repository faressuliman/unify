...
const getFallbackAge = (details: string, fallback: string) => {
  const match = details.match(/(\d+\s*(?:years old|year old|سنة|سنين|عاماً))/i);
...
export default function MissingModal({ isOpen, onOpenChange, profile, isRTL }: MissingModalProps) {
  ...
  const lastSeenLocation = toRTLDisplay(profile.lastSeenLocationDetails || profile.location, isRTL);
  const city = toRTLDisplay(profile.city || t.notAvailable, isRTL);
  const postedBy = toArabicDisplay(profile.postedBy || t.unknown, isRTL);
        ...
        <DialogPrimitive.Portal>
          ...
          >
            ...
            <div className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-100 bg-white px-5 py-4 sm:px-8 sm:py-5">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-3">
                  <span className="flex h-2.5 w-2.5 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]"></span>
            ...
            <div className="space-y-6 sm:space-y-8 p-5 sm:p-8">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <InfoCard icon={<MapPin className="h-4.5 w-4.5" />} label={t.lastSeenLocation} value={lastSeenLocation} fullWidth />
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[20px] bg-red-50/60 p-5 sm:p-6 border border-red-100/60">
                <div className="flex items-start gap-4">
                  <div className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-100 text-red-600">
                      <ShieldAlert className="h-5 w-5" />
                    </div>
                    <p className="text-[14px] sm:text-[15px] font-medium leading-relaxed text-red-900/80">
                      {t.sightingSummary}
                    </p>
                  </div>
                </div>
