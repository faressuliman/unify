...
const HomeFeatureCard = memo(function HomeFeatureCard({
  ...
  const cardContent = (
    <div className={`relative z-10 bg-white border border-slate-200 rounded-xl shadow-sm transition-all duration-300 hover:shadow-md ${isMobile
      ...
      <motion.p 
        ...
        className={`text-slate-500 leading-relaxed ${isMobile
        ? "text-xs mb-4 flex-1"
        ...
        <div className={`flex justify-center gap-2 mt-auto pt-2 ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
          {Array.from({ length: totalCards }).map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${index === currentIndex ? "bg-secondary w-6" : "bg-slate-200 w-2"
                }`}
  ...
  if (isMobile) {
    return (
      <div className="w-full shrink-0 px-2 lg:px-4">
        {cardContent}
      </div>
    );
  }
