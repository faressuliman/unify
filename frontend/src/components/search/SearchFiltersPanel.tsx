...
export default function SearchFiltersPanel({
  ...
  const eyeColorOptions = useMemo(
    () => [{ value: "", label: t("search.any") || "Any" }, ...eyeColors],
    [eyeColors, t],
  );

  const hairColorOptions = useMemo(
    () => [{ value: "", label: t("search.any") || "Any" }, ...hairColors],
    [hairColors, t],
  );
    ...
    if (!hasAnyFilter && !searchImage) {
      setSubmitError(
        t("search.atLeastOneInputError") ||
          (isRTL
            ? "يرجى إدخال حقل واحد على الأقل قبل البحث."
            : "Please enter at least one input before searching."),
      );
      return;
    }
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <FormInput
              ...
              label={renderFieldLabel(
                t("search.firstName") || "First Name",
                "firstName",
              )}
              placeholder={
                t("search.firstNamePlaceholder") || "Enter first name"
              }
            ...
            <Controller
              ...
              render={({ field }) => (
                <LocalizedDateInput
                  id="dateMissing"
                  label={renderFieldLabel(
                    t("search.dateMissing") || "Date They Went Missing",
                    "dateMissing",
                  )}
                  value={field.value}
                  onChange={field.onChange}
                  isRTL={isRTL}
                  placeholder={
                    t("search.dateInputHint") ||
                    (isRTL ? "انقر لإدخال التاريخ" : "Click to enter the date")
                  }
                  labelClassName={labelClass}
                />
              )}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <FormInput
              id="location"
              label={renderFieldLabel(
                t("search.location") || "Location details",
                "location",
              )}
              placeholder={
                t("search.locationPlaceholder") || "Enter specific location..."
              }
              labelClassName={labelClass}
              {...register("location")}
            />
          ...
          <FormTextArea
            id="clothing"
            label={renderFieldLabel(
              t("search.clothing") || "Clothing Description",
              "clothing",
            )}
            placeholder={
              t("search.clothingPlaceholder") ||
